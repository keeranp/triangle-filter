...
def rmse(img1, img2):
    ...
    rmserr = math.sqrt(reduce(operator.add,map(lambda h, i: h*(i**2), h, range(256))) / (float(img1.size[0]) * img1.size[1]))
    return rmserr
...
real_image = Image.open('img\\landscape.jpg')
...
i = 1
successful_iteration = 0
final_image.save('out\\out_0.jpg')
...
final_image.show()
plt.plot(np.arange(0,i-1,1),SUCCESSFUL_ITERATION)
plt.show()